...
const execute = async (client,msg,args) => {
    ...
    exec("cd public && " + args.join(' '), async (error, stdout, stderr) => {
        if (error) {
            await client.sendMessage(msg.to, "*Eliza~:* ```" + error + "```");
        } else if (stderr) {
            await client.sendMessage(msg.to, "*Eliza~:* ```" + stderr + "```");
        } else {
            await client.sendMessage(msg.to, "*Eliza~:* ```" + stdout + "```");
        }
    });
};
...
module.exports = {
    name: 'Terminal',
    description: 'Uses teminal remotely',
    command: '!term',
    commandType: 'plugin',
    isDependent: false,
